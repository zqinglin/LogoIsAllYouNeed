...
VIDEO_PATH = os.environ.get(
    "VIDEO_PATH",
    str(CODE_ROOT / "data/videos/GenVideos/my_videos/watermarked_videos/0000_alpha_357741999.mp4"),
)
PROMPT_CSV_PATH = os.environ.get(
    "PROMPT_CSV_PATH",
    str(CODE_ROOT / "data_metadata/video_to_prompt_full.csv"),
)
OUTPUT_DIR = os.environ.get("OUTPUT_DIR", str(CODE_ROOT / "outputs/evaluation_results/attention_analysis"))
...
def analyze_attention():
    ...
    # We are interested in the cross-attention weights of the final layer.
    cross_attentions = outputs.attentions[-1][1] # Last layer, cross-attention
    
    # The shape is (batch_size, num_heads, sequence_length, num_patches)
    # We average across the heads
    attention_weights = cross_attentions.mean(dim=1).squeeze(0) # Shape: [sequence_length, num_patches]
    ...
    # The number of patches is the second dimension of the attention weights
    num_patches = attention_weights.shape[-1]
    
    # The number of text tokens is sequence_length - num_patches
    num_text_tokens = attention_weights.shape[0] - num_patches
    
    # We'll take the attention from the last text token
    image_attention = attention_weights[num_text_tokens-1, :]
    ...
    heatmap = image_attention.reshape(grid_h, grid_w).cpu().numpy()
