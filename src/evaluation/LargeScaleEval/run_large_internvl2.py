...
MODEL_PATH = "OpenGVLab/InternVL2-8B" # Update if local path is needed
MAX_FRAMES = 8
NUM_PASSES = 3
...
OUTPUT_CSV = os.environ.get(
    "OUTPUT_CSV",
    str(CODE_ROOT / "outputs/LargeScaleEval/results_internvl2_with_variance.csv"),
)
# ================================
...
def evaluate_video(video_path, prompt):
    ...
    try:
        ...
        for pass_idx in range(NUM_PASSES):
            ...
            question = f"<image>\n" * len(frames) + UNIFIED_VLM_PROMPT.replace("{text_prompt}", prompt)
            
            generation_config = dict(max_new_tokens=150, do_sample=True, temperature=0.7, top_p=0.9)
...
def main():
    ...
    print(f"Starting InternVL2 inference on {len(tasks)} videos...")
    ...
    if len(tasks) == 0:
        ...
        with ProcessPoolExecutor(max_workers=len(GPU_DEVICES), initializer=init_worker, initargs=(gpu_queue,), mp_context=__import__("multiprocessing").get_context("spawn")) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="InternVL2 Eval (mean/std)"):
