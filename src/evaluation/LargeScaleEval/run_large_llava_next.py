...
OUTPUT_CSV = os.environ.get(
    "OUTPUT_CSV",
    str(CODE_ROOT / "outputs/LargeScaleEval/results_llava_next_with_variance.csv"),
)
# ================================
...
def init_worker(gpu_queue):
    ...
    print(f"Loading LLaVA-NeXT on GPU {worker_gpu}...")
...
def main():
    ...
    print(f"Starting LLaVA-NeXT inference on {len(tasks)} videos...")
    ...
    if len(tasks) == 0:
        ...
        with ProcessPoolExecutor(max_workers=len(GPU_DEVICES), initializer=init_worker, initargs=(gpu_queue,), mp_context=__import__("multiprocessing").get_context("spawn")) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="LLaVA-NeXT Eval (mean/std)"):
